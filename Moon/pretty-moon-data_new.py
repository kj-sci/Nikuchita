...
class pretty_moon_data:
	...
	def process_00(self):
		width = 300
		height = 300
		self.init_new_image(width, height)
		centroid_out = [100, 150]
		radius = 50
		write('centroid_out: '+str(centroid_out[0])+', '+str(centroid_out[1])+'\n')
		self.make_dummy_image(centroid_out, radius)
		self.save_image(self.img_fname_out)
	...
	# copy image, background: paint with black
	def process_4(self):
		self.read_img()
	
		self.centroid = [int(self.size_in[0]/2+0.5), int(self.size_in[1]/2+0.5)]
		self.radius = 117
	...
	def process_20(self):
		...
		self.init_new_image(width, height)
		centroid_in = [100, 150]
		#centroid_in[0] = int(self.size_in[0]/2.0+0.5)
		#centroid_in[1] = int(self.size_in[1]/2.0+0.5)
		write('centroid_in: '+str(centroid_in[0])+', '+str(centroid_in[1])+'\n')
		self.rotate_image(centroid_in, np.pi/6.0)
		self.save_image(self.img_fname_out)
	...
	def init_radius(self):
		...
		if max_radius < self.size_in[0] - self.centroid[0]:
			max_radius = self.size_in[0] - self.centroid[0]
		if max_radius < self.size_in[1] - self.centroid[1]:
			max_radius = self.size_in[1] - self.centroid[1]
	...
	def init_copy_image(self):
		self.init_new_image(self.size_in[0], self.size_in[1])
	...
	def copy_image_2(self):
		radius_sqr = self.radius * self.radius
		for x in range(self.size_in[0]):
			for y in range(self.size_in[1]):
				r_sqr = pow(x-self.centroid[0], 2) + pow(y-self.centroid[1], 2)
				if r_sqr >= radius_sqr:
	...
	def rotate_image(self, centroid_in, theta):
		centroid_out = [0, 0]
		for loop in range(2):
			centroid_out[loop] = int(self.size_out[loop]/2.0 + 0.5)
		write('centroid_out: '+str(centroid_out[0])+', '+str(centroid_out[1])+'\n')
		
		for x_out in range(self.size_out[0]):
			dx_out = x_out - centroid_out[0]
			for y_out in range(self.size_out[1]):
				dy_out = y_out - centroid_out[1]
				x_in = int(centroid_in[0] + dx_out*np.cos(theta) - dy_out*np.sin(theta) + 0.5)
				y_in = int(centroid_in[1] + dx_out*np.sin(theta) + dy_out*np.cos(theta) + 0.5)
				#write('(x_in, y_in) = ('+str(x_in)+', '+str(y_in)+')\n')
				#write('(x_out, y_out) = ('+str(x_out)+', '+str(y_out)+')\n')
				
				if x_in < 0 or y_in < 0 or x_in >= self.size_in[0] or y_in >= self.size_in[1]:
					self.image_out.putpixel((x_out,y_out), (0, 0, 0, 0))
				else:
					self.pixel = self.image_in.getpixel((x_in,y_in))
					self.image_out.putpixel((x_out,y_out), (self.pixel[0], self.pixel[1], self.pixel[2], 0))
	...
	def make_moon_image(self, centroid):
		...
		for dx_0 in range(self.size_out[0]):
			...
			for dy_0 in range(self.size_out[1]):
				...
				y_in = centroid[1] + dy
				if x_in >= 0 and y_in >= 0 and x_in < self.size_in[0] and y_in < self.size_in[1]:
					self.pixel = self.image_in.getpixel((x_in,y_in))
				else:
					self.pixel = [0, 0, 0]
